...
export function isAllowedGatewayUrl(
  url: string,
  whitelist: readonly string[] = DEFAULT_CF_GATEWAY_PREFIXES,
): boolean {
 ...
 */
export function assertGatewayEgress(
  ctx: ProviderContext,
  ...
  if (!hasGatewayId && !baseUrl) {
    throw new LlmKitError(
      "egress_not_allowed",
      `provider "${ctx.name}" must route through an AI Gateway (missing gatewayId or http.baseUrl)`,
    );
  }
  ...
  if (baseUrl && !isAllowedGatewayUrl(baseUrl, whitelist)) {
    throw new LlmKitError(
      "egress_not_allowed",
      `provider "${ctx.name}" base URL "${baseUrl}" is not an allowed AI Gateway / CF REST endpoint`,
    );
  }
}
